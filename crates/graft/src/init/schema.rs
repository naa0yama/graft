use std::path::Path;

use anyhow::Context as _;

/// Outcome of a `write_schema_file` call.
#[derive(Debug, PartialEq, Eq)]
pub enum WriteOutcome {
    /// The file did not exist and was created.
    Created,
    /// The file existed with different content and was overwritten.
    Updated,
    /// The file existed with identical content; no write was performed.
    Unchanged,
}

/// JSON Schema for the graft manifest format (yaml-language-server compatible).
pub const SCHEMA_JSON: &str = r#"{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "title": "graft manifest",
  "description": "Template sync configuration for graft",
  "type": "object",
  "required": ["upstream", "files"],
  "additionalProperties": false,
  "properties": {
    "upstream": {
      "type": "object",
      "required": ["repo"],
      "additionalProperties": false,
      "properties": {
        "repo": {
          "type": "string",
          "pattern": "^[a-zA-Z0-9._-]+/[a-zA-Z0-9._-]+$",
          "description": "GitHub repository in owner/name format"
        },
        "ref": {
          "type": "string",
          "default": "main",
          "description": "Git ref (branch, tag, or commit SHA)"
        }
      }
    },
    "spec": {
      "type": "object",
      "additionalProperties": false,
      "description": "Repository settings — compatible (all fields optional)",
      "properties": {
        "description": {
          "type": "string",
          "description": "Short description of the repository"
        },
        "homepage": {
          "type": "string",
          "description": "Repository homepage URL"
        },
        "visibility": {
          "type": "string",
          "enum": ["public", "private", "internal"],
          "description": "Repository visibility"
        },
        "archived": {
          "type": "boolean",
          "description": "Whether the repository is archived"
        },
        "topics": {
          "type": "array",
          "items": { "type": "string" },
          "description": "Topic tags (full list, not additive)"
        },
        "web_commit_signoff_required": {
          "type": "boolean",
          "description": "Require contributors to sign off on web-based commits"
        },
        "release_immutability": {
          "type": "boolean",
          "description": "Lock releases after publishing"
        },
        "label_sync": {
          "type": "string",
          "enum": ["additive", "mirror"],
          "description": "Label sync mode: additive (default) keeps unmanaged labels; mirror deletes them"
        },
        "labels": {
          "type": "array",
          "description": "Managed repository labels",
          "items": {
            "type": "object",
            "required": ["name", "color"],
            "additionalProperties": false,
            "properties": {
              "name": { "type": "string", "minLength": 1, "description": "Label name (unique)" },
              "color": { "type": "string", "pattern": "^[0-9a-fA-F]{6}$", "description": "Hex colour without #" },
              "description": { "type": "string", "description": "Human-readable description" }
            }
          }
        },
        "features": {
          "type": "object",
          "additionalProperties": false,
          "description": "Repository feature toggles",
          "properties": {
            "issues":      { "type": "boolean" },
            "projects":    { "type": "boolean" },
            "wiki":        { "type": "boolean" },
            "discussions": { "type": "boolean" }
          }
        },
        "merge_strategy": {
          "type": "object",
          "additionalProperties": false,
          "description": "Merge strategy settings",
          "properties": {
            "allow_merge_commit":           { "type": "boolean" },
            "allow_squash_merge":           { "type": "boolean" },
            "allow_rebase_merge":           { "type": "boolean" },
            "allow_auto_merge":             { "type": "boolean", "description": "Automatically merge when all checks pass" },
            "allow_update_branch":          { "type": "boolean", "description": "Allow updating PR branches" },
            "auto_delete_head_branches":    { "type": "boolean" },
            "merge_commit_title":          { "type": "string", "enum": ["PR_TITLE", "MERGE_MESSAGE"], "description": "Title format for merge commits" },
            "merge_commit_message":        { "type": "string", "enum": ["PR_BODY", "PR_TITLE", "BLANK"], "description": "Body format for merge commits" },
            "squash_merge_commit_title":   { "type": "string", "enum": ["PR_TITLE", "COMMIT_OR_PR_TITLE"], "description": "Title format for squash commits" },
            "squash_merge_commit_message": { "type": "string", "enum": ["PR_BODY", "COMMIT_MESSAGES", "BLANK"], "description": "Body format for squash commits" }
          }
        },
        "actions": {
          "type": "object",
          "additionalProperties": false,
          "description": "GitHub Actions settings (enabled must be set when any other field is present)",
          "properties": {
            "enabled": {
              "type": "boolean",
              "description": "Must be set when any other actions.* field is present"
            },
            "allowed_actions": {
              "type": "string",
              "enum": ["all", "local_only", "selected"],
              "description": "Which actions are allowed to run"
            },
            "sha_pinning_required": {
              "type": "boolean",
              "description": "Require full SHA pinning for external actions"
            },
            "workflow_permissions": {
              "type": "string",
              "enum": ["read", "write"],
              "description": "Default GITHUB_TOKEN permission level"
            },
            "can_approve_pull_requests": {
              "type": "boolean",
              "description": "Allow workflows to approve pull requests"
            },
            "selected_actions": {
              "type": "object",
              "additionalProperties": false,
              "description": "Allowed action patterns (requires allowed_actions: selected)",
              "properties": {
                "github_owned_allowed": { "type": "boolean" },
                "patterns_allowed": {
                  "type": "array",
                  "items": { "type": "string" }
                }
              }
            },
            "fork_pr_approval": {
              "type": "string",
              "enum": [
                "first_time_contributors_new_to_github",
                "first_time_contributors",
                "all_external_contributors"
              ],
              "description": "Fork pull request approval policy"
            }
          }
        },
        "rulesets": {
          "type": "array",
          "description": "Repository rulesets (preferred over branch_protection)",
          "items": {
            "type": "object",
            "required": ["name"],
            "additionalProperties": false,
            "properties": {
              "name": { "type": "string", "minLength": 1, "description": "Unique ruleset name" },
              "target": {
                "type": "string",
                "enum": ["branch", "tag"],
                "description": "Whether the ruleset targets branches or tags"
              },
              "enforcement": {
                "type": "string",
                "enum": ["active", "disabled", "evaluate"],
                "description": "Enforcement level"
              },
              "bypass_actors": {
                "type": "array",
                "description": "Actors that may bypass this ruleset",
                "items": {
                  "type": "object",
                  "additionalProperties": false,
                  "properties": {
                    "role":         { "type": "string", "enum": ["admin", "maintain", "write", "triage", "read"], "description": "Built-in repository role that may bypass the ruleset" },
                    "team":         { "type": "string" },
                    "app":          { "type": "string" },
                    "org-admin":    { "type": "boolean" },
                    "custom-role":  { "type": "string" },
                    "bypass_mode": {
                      "type": "string",
                      "enum": ["always", "pull_request", "exempt"]
                    }
                  }
                }
              },
              "conditions": {
                "type": "object",
                "additionalProperties": false,
                "properties": {
                  "ref_name": {
                    "type": "object",
                    "additionalProperties": false,
                    "properties": {
                      "include": { "type": "array", "items": { "type": "string" } },
                      "exclude": { "type": "array", "items": { "type": "string" } }
                    }
                  }
                }
              },
              "rules": {
                "type": "object",
                "additionalProperties": false,
                "properties": {
                  "non_fast_forward":        { "type": "boolean", "description": "Block force pushes" },
                  "deletion":                { "type": "boolean", "description": "Block ref deletion" },
                  "creation":                { "type": "boolean", "description": "Block ref creation" },
                  "required_linear_history": { "type": "boolean", "description": "Require linear commit history" },
                  "required_signatures":     { "type": "boolean", "description": "Require signed commits" },
                  "pull_request": {
                    "type": "object",
                    "additionalProperties": false,
                    "properties": {
                      "required_approving_review_count": { "type": "integer", "minimum": 0 },
                      "dismiss_stale_reviews_on_push":   { "type": "boolean" },
                      "require_code_owner_review":       { "type": "boolean" },
                      "require_last_push_approval":      { "type": "boolean" },
                      "required_review_thread_resolution": { "type": "boolean" },
                      "allowed_merge_methods": {
                        "type": "array",
                        "items": {
                          "type": "string",
                          "enum": ["squash", "merge", "rebase"]
                        },
                        "description": "Merge methods allowed for pull requests"
                      }
                    }
                  },
                  "required_status_checks": {
                    "type": "object",
                    "additionalProperties": false,
                    "properties": {
                      "strict_required_status_checks_policy": { "type": "boolean" },
                      "contexts": {
                        "type": "array",
                        "items": {
                          "type": "object",
                          "required": ["context"],
                          "additionalProperties": false,
                          "properties": {
                            "context":        { "type": "string" },
                            "app":            { "type": "string" },
                            "integration_id": { "type": "integer", "description": "GitHub App integration ID resolved from the app slug" }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "branch_protection": {
          "type": "array",
          "description": "Classic branch protection rules (prefer rulesets for new configurations)",
          "items": {
            "type": "object",
            "required": ["pattern"],
            "additionalProperties": false,
            "properties": {
              "pattern":                   { "type": "string", "minLength": 1, "description": "Branch name pattern (unique)" },
              "required_reviews":          { "type": "integer", "minimum": 0 },
              "dismiss_stale_reviews":     { "type": "boolean" },
              "require_code_owner_reviews":{ "type": "boolean" },
              "require_status_checks": {
                "type": "object",
                "additionalProperties": false,
                "properties": {
                  "strict":   { "type": "boolean" },
                  "contexts": { "type": "array", "items": { "type": "string" } }
                }
              },
              "enforce_admins":     { "type": "boolean" },
              "allow_force_pushes": { "type": "boolean" },
              "allow_deletions":    { "type": "boolean" }
            }
          }
        }
      }
    },
    "files": {
      "type": "array",
      "minItems": 1,
      "description": "Synchronisation file rules (must be non-empty)",
      "items": {
        "type": "object",
        "required": ["path", "strategy"],
        "additionalProperties": false,
        "properties": {
          "path": {
            "type": "string",
            "minLength": 1,
            "description": "Destination path in the downstream repository (relative, no leading /)"
          },
          "strategy": {
            "type": "string",
            "enum": ["replace", "create_only", "delete", "patch"],
            "description": "Synchronisation strategy"
          },
          "source": {
            "type": "string",
            "minLength": 1,
            "description": "Override for the upstream source path (replace/create_only only)"
          },
          "patch": {
            "type": "string",
            "minLength": 1,
            "description": "Explicit patch file path (patch strategy only; defaults to .github/graft/patches/<path>.patch)"
          },
          "preserve_markers": {
            "type": "boolean",
            "description": "Preserve graft:keep-start/keep-end marker blocks from the local file. Only used when strategy is 'patch' or 'replace'."
          }
        }
      }
    }
  }
}"#;

/// Returns the yaml-language-server schema comment line.
///
/// Prepend this to generated config files to enable editor autocompletion.
#[must_use]
pub const fn comment() -> &'static str {
    "# yaml-language-server: $schema=./schema.json\n"
}

/// Write `schema.json` to `dir`, always ensuring the file matches `SCHEMA_JSON`.
///
/// # Errors
///
/// Returns an error when the file cannot be read or written.
pub fn write_schema_file(dir: &Path) -> anyhow::Result<WriteOutcome> {
    let dest = dir.join("schema.json");
    let outcome = if dest.exists() {
        let existing = std::fs::read_to_string(&dest)
            .with_context(|| format!("failed to read '{}'", dest.display()))?;
        if existing == SCHEMA_JSON {
            return Ok(WriteOutcome::Unchanged);
        }
        WriteOutcome::Updated
    } else {
        WriteOutcome::Created
    };
    std::fs::write(&dest, SCHEMA_JSON)
        .with_context(|| format!("failed to write '{}'", dest.display()))?;
    Ok(outcome)
}

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------

#[cfg(test)]
mod tests {
    #![allow(clippy::unwrap_used)]

    use tempfile::TempDir;

    use super::*;

    #[test]
    fn schema_json_is_valid_json() {
        serde_json::from_str::<serde_json::Value>(SCHEMA_JSON).unwrap();
    }

    #[test]
    fn schema_comment_contains_schema_json() {
        assert!(comment().contains("schema.json"));
        assert!(comment().contains("yaml-language-server"));
    }

    #[test]
    fn write_schema_file_creates_file() {
        let dir = TempDir::new().unwrap();
        let outcome = write_schema_file(dir.path()).unwrap();
        assert_eq!(outcome, WriteOutcome::Created);
        let content = std::fs::read_to_string(dir.path().join("schema.json")).unwrap();
        serde_json::from_str::<serde_json::Value>(&content).unwrap();
    }

    #[test]
    fn write_schema_file_overwrites_stale_content() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("schema.json");
        std::fs::write(&path, b"stale").unwrap();
        let outcome = write_schema_file(dir.path()).unwrap();
        assert_eq!(outcome, WriteOutcome::Updated);
        let content = std::fs::read_to_string(&path).unwrap();
        assert_eq!(content, SCHEMA_JSON);
    }

    #[test]
    fn write_schema_file_returns_unchanged_when_identical() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("schema.json");
        std::fs::write(&path, SCHEMA_JSON).unwrap();
        let outcome = write_schema_file(dir.path()).unwrap();
        assert_eq!(outcome, WriteOutcome::Unchanged);
        let content = std::fs::read_to_string(&path).unwrap();
        assert_eq!(content, SCHEMA_JSON);
    }
}
